...
public class SearchQuery extends PagingQuery implements WhereClause {
    ...
    public <T extends Object> T get(String key, Class<T> classType){
        ...
        switch (type){
            ...
            case JSON:
                try {
                    result = MessageParser.unmarshal(classType, value);
                } catch (IOException e) {
                    e.printStackTrace();
                }
                break;
    ...
    public Predicate between(Object aVal, Object bVal) {
        ...
        //updateCurrentProperty(null, Operator.BETWEEN);
        System.out.println("SearchQuery::between() not implemented!");
        return getPredicate();
    }
    ...
    public Predicate notBetween(Object aVal, Object bVal) {
        //TODO: Not Implemented
        //updateCurrentProperty(null, Operator.NOT_BETWEEN);
        System.out.println("SearchQuery::notBetween() not implemented!");
        return getPredicate();
    }
    ...
    private class QueryPredicate implements Predicate {
        ...
        @Override
        public Predicate and(Expression expression) {
            return this;
        }

        @Override
        public Predicate or(Expression expression) {
            return this;
        }

        private Predicate createExpression(QueryProperty property, Logic logic) {
            Expression exp = new ExpressionProxy(new Property(property.getKey(), property.getValue()), property.getOperator());
            return create(exp, logic);
        }

        private Predicate create(Expression exp, Logic logic){
            if(expression == null) {
                expression = exp;
            }
            if(logic != null) {
        ...
        private void createAnd(Expression exp) {
            expression = new And(expression, exp);
        }

        private void createOr(Expression exp) {
            expression = new Or(expression, exp);
        }
        ...
        public ExpressionProxy[] resolve() {
            ...
            return (expression != null) ? expression.resolve() : new ExpressionProxy[0];
        }
    }
